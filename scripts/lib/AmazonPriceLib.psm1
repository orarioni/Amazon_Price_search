        ...
        try {
            ...
            if ($detail.Class -eq 'Auth' -and $AuthContext) {
                try {
                    ...
                    $res = Invoke-WithRetry -Label "Catalogバッチ再取得(認証更新) ($($i + 1)/$($chunks.Count))" -MaxRetries $Config.MaxRetries -LogPath $LogPath -Action {
                        Invoke-RestMethod -Method Get -Uri $uri -Headers @{
                            'Authorization'      = "Bearer $AccessToken"
                            'x-amz-access-token' = $AccessToken
                            'User-Agent'         = $Config.UserAgent
    ...
    for ($i = 0; $i -lt $chunks.Count; $i++) {
        ...
        try {
            ...
            if ($detail.Class -eq 'Auth' -and $AuthContext) {
                try {
                    $AccessToken = Get-LwaAccessTokenCached -ClientId $AuthContext.ClientId -ClientSecret $AuthContext.ClientSecret -RefreshToken $AuthContext.RefreshToken -Config $Config -LogPath $LogPath -TokenCachePath $AuthContext.TokenCachePath -ForceRefresh
                    $res = Invoke-WithRetry -Label "Pricingバッチ再取得(認証更新) ($($i + 1)/$($chunks.Count))" -MaxRetries $Config.MaxRetries -LogPath $LogPath -Action {
                        Invoke-RestMethod -Method Post -Uri "$($Config.SpApiBaseUrl)/batches/products/pricing/v0/itemOffers" -Headers @{
                            'Authorization'      = "Bearer $AccessToken"
                            'x-amz-access-token' = $AccessToken
                            'User-Agent'         = $Config.UserAgent
...
Export-ModuleMember -Function ConvertTo-PlainText,Write-Log,Classify-StatusAndBody,Get-ErrorDetail,Invoke-WithRetry,Split-IntoChunks,Read-AccessTokenCache,Save-AccessTokenCache,Get-LwaAccessTokenCached,Get-LwaAccessToken,Get-LowestNewPriceFromOffers,Get-AsinMapByJanBatch,Get-PriceMapByAsinBatch,Load-PersistentCache,Save-PersistentCache,Append-DailyPriceHistory,Is-CacheFresh,Save-SecretsInteractive,Invoke-AmazonPriceUpdate
