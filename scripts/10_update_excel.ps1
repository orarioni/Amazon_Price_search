...
function Get-LowestNewPrice {
    ...
    foreach ($offer in $res.payload.Offers) {
        ...
        if ($offer.ListingPrice -and $offer.Shipping) {
            $price = [decimal]$offer.ListingPrice.Amount + [decimal]$offer.Shipping.Amount
        }

        if ($offer.LandedPrice) {
            $landed = [decimal]$offer.LandedPrice.Amount
            if ($null -eq $price -or $landed -lt $price) {
                $price = $landed
            }
        }

        if ($null -ne $price -and ($null -eq $minPrice -or $price -lt $minPrice)) {
            $minPrice = $price
        }
    }
